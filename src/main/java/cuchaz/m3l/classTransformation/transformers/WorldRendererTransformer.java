...
public class WorldRendererTransformer implements ClassTransformer {
    ...
    public static Optional<ChunkSectionRenderer> getChunkSectionRendererNeighbor(WorldRenderer worldRenderer, BlockPos pos, ChunkSectionRenderer chunkSectionRenderer, Facing facing) {
        ChunkSystem chunkSystem = M3L.INSTANCE.getRegistry().chunkSystem.get().get();
        if (chunkSystem != null) {
            return Optional.of(chunkSystem.getChunkSectionRendererNeighbor(worldRenderer, pos, chunkSectionRenderer, facing));
        }
