...
import cuchaz.m3l.util.MinecraftUtil;
...
public class ItemTransformer implements ClassTransformer {
    ...
    public static Item onGetItemFromBlock(Block block) {
        ...
        if (MinecraftUtil.isModBlock(blockNumber)) {
            return Item.getItem(MinecraftUtil.getModItemNumberFromModBlockNumber(blockNumber));
        } else {
        ...
        method.instrument(new ExprEditor() {
            ...
            public void edit(MethodCall call) throws CannotCompileException {
                if (call.getClassName().equals(RegistryNamespaced.class.getName()) && call.getMethodName().equals("addObject")) {
                    call.replace("if( " + MinecraftUtil.class.getName() + ".isVanillaBlock( $1 ) || !( " + ItemBlock.class.getName() + ".class.isAssignableFrom( $3.getClass() ) ) ) { $proceed( $$ ); }");
                }
            }
        });
