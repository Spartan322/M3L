...
public class BytecodeIndexIterator implements Iterator<BytecodeIndexIterator.Index> {
    ...
    public boolean hasNext() {
        return m_next.isPresent() && m_next != Optional.<Index>empty();
    }

    @Override
    public Index next() {
        Index out = m_next.get();
        try {
            m_next = getNext();
    ...
    private Optional<Index> getNext() throws BadBytecode {
        while (m_iter.hasNext()) {
            ...
            switch (opcode) {
                ...
                case Opcode.GETSTATIC:
                    return Optional.of(new Index(m_iter, pos + 1, true));

                case Opcode.LDC:
                    return Optional.of(new Index(m_iter, pos + 1, false));
            }
        }
