...
public enum ClassType {
    ...
    ClassType(boolean shouldTranslate, boolean shouldHook, boolean shouldFilter, String... packages) {
    ...
    private static Optional<String> getPackage(String nameJava) {
        int index = nameJava.lastIndexOf('.');
        if (index == -1) {
            return Optional.empty();
        }
        return Optional.of(nameJava.substring(0, index));
    }
    ...
    public boolean includes(String nameJava) {
        ...
        while ((thePackage = getPackage(thePackage).orElse(null)) != null) {
